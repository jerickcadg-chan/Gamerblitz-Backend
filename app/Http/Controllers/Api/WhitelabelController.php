...
{
    /**
     * Handle Whitelabel order callback.
     * 
    ...
    public function orderCallback(Request $request, OrderService $orderService)
    {
        ...
        if ($providedToken !== Setting::getByKey('whitelabel_callback_token')) {
        ...
        try {
            $log->notice('📩 Whitelabel callback received', $payload);
    ...
    private function handleRefund(Order $order, array $payload, OrderService $orderService, string $note): void
    {
        ...
        if ($order->user && $order->user->balance) {
            ...
            if ($balance) {
                ...
                Log::channel('whitelabel')->notice("💰 Balance refunded for order {$order->code}");
            }
        }
    }
}
