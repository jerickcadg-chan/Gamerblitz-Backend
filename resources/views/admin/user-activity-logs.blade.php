@extends('layouts.app', [
    'activePage' => 'user-activity-logs',
])

@section('content')
  <div class="page-header">
    <h3 class="page-title"> User Activity Logs </h3>
    <nav aria-label="breadcrumb">
      <ol class="breadcrumb">
        <li class="breadcrumb-item"><a href="{{ route('user-activity-logs.index') }}">User Activity Logs</a></li>
  ...
  <div class="col-lg-12 grid-margin stretch-card">
    <div class="card">
      <div class="card-body table-responsive">
        <form method="GET" action="{{ route('user-activity-logs.index') }}" class="mb-3">
          <div class="row">
            @include('master.date-range', [
                'col' => 'col-xl-3 col-md-6 mb-2',
            ])
            <div class="col-xl-3 col-md-6 col-sm-12 mb-2">
              <select class="form-control" id="type" name="type">
                <option value="">All Users</option>
                <option value="customer" {{ request('type') === 'customer' ? 'selected' : '' }}>Customer</option>
                <option value="non_customer" {{ request('type') === 'non_customer' ? 'selected' : '' }}>Non-Customer
                </option>
          ...
          <div class="row">
            <div class="col-12 d-flex justify-content-end mb-2">
              ...
              @if (request('type') || request('search') || request('ip') || request('dates'))
                <a href="{{ route('user-activity-logs.index') }}" class="btn btn-secondary ms-2">Clear</a>
              @endif
            </div>
          </div>
        </form>
          ...
          <tbody>
            @forelse ($logs as $index => $log)
              <tr>
                ...
                <td>{{ $log->ip_address }} @if ($log->is_banned)
