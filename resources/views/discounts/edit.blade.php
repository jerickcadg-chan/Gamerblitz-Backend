            ...
            <div class="card-body">
                <form method="POST" action="{{ $updateLink }}">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="name" class="form-control {{ $errors->has('name') ? ' is-invalid' : '' }}" id="input_name" placeholder="Masukkan nama" value="{{ old('name', $discount->name) }}" required>
                        @include('alerts.feedback', ['field' => 'name'])
                    </div>
                    <div class="form-group">
                        <label for="input_code">Kode Diskon</label>
                        <input type="text" name="code" class="form-control {{ $errors->has('code') ? ' is-invalid' : '' }}" id="input_code" placeholder="Kosongi apabila tidak ada kode" value="{{ old('code', $discount->code) }}">
                    ...
                    <div class="row">
                        <div class="form-group col-md-3">
                            ...
                            <select class="form-control {{ $errors->has('disc_type') ? ' is-invalid' : '' }}" id="input_disc_type" name="disc_type" required>
                                @foreach (config('array.discount.disc_type') as $disc)
                                    <option value="{{ $disc['value'] }}" {{ old('disc_type', $discount->disc_type) == $disc['value'] ? 'selected' : null }}>{{ $disc['desc'] }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group col-md-5">
                            ...
                            <input type="number" name="nominal" class="form-control {{ $errors->has('nominal') ? ' is-invalid' : '' }}" id="input_nominal" placeholder="Masukkan nominal sesuai jenis diskon" value="{{ old('nominal', $discount->nominal) }}" required>
                            @include('alerts.feedback', ['field' => 'nominal'])
                        </div>
                        <div class="form-group col-md-4">
                            <label for="input_maximum" class="required">Maksimal Penggunaan</label>
                            <input type="number" min="1" name="maximum" class="form-control {{ $errors->has('maximum') ? ' is-invalid' : '' }}" id="input_maximum" placeholder="Masukkan maksimal penggunaan diskon" value="{{ old('maximum', $discount->maximum) }}" required>
                    ...
                    <div class="row">
                        <div class="form-group col-md-6">
                            ...
                            <input type="date" name="start_date" class="form-control {{ $errors->has('start_date') ? ' is-invalid' : '' }}" id="input_start_date" placeholder="Masukkan tanggal mulai diskon" value="{{ old('start_date', parse_date_format($discount->start_date)) }}" required>
                            @include('alerts.feedback', ['field' => 'start_date'])
                        </div>
                        <div class="form-group col-md-6">
                            <label for="input_end_date" class="required">Tanggal Selesai</label>
                            <input type="date" name="end_date" class="form-control {{ $errors->has('end_date') ? ' is-invalid' : '' }}" id="input_end_date" placeholder="Masukkan tanggal berakhir diskon" value="{{ old('end_date', parse_date_format($discount->end_date)) }}" required>
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control {{ $errors->has('product_type') ? ' is-invalid' : '' }}" id="input_product_type" name="product_type" onchange="checkProductType()" required>
                            @foreach (config('array.discount.product_type') as $disc)
                                <option value="{{ $disc['value'] }}" {{ old('product_type', $discount->product_type) == $disc['value'] ? 'selected' : null }}>{{ $disc['desc'] }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div id="product_type_div" style="display: none">
                        ...
                        @foreach (\App\Models\Product::all() as $product)
                            <div class="item">
                                <input type="checkbox" name="product_id[]" value="{{ $product->id }}" class="my-2" {{ (is_array($discount->products->toArray()) && in_array($product->id, $discount->products->pluck('productable_id')->toArray()) && in_array('App\Models\Product', $discount->products->pluck('productable_type')->toArray())) ? ' checked' : '' }}>
                    ...
                    <div id="product_item_div" style="display: none">
                        ...
                        @foreach (\App\Models\ProductItem::all() as $product_item)
                            <div class="item">
                                <input type="checkbox" name="product_item_id[]" value="{{ $product_item->id }}" class="my-2" {{ (is_array($discount->products->toArray()) && in_array($product_item->id, $discount->products->pluck('productable_id')->toArray()) && in_array('App\Models\ProductItem', $discount->products->pluck('productable_type')->toArray())) ? ' checked' : '' }}>
